...
gkVehicle::gkVehicle(gkScene* scene)
	: m_scene(scene), m_dynamicWorld(0), m_raycaster(0), m_vehicle(0), m_object(0), m_chassis(0),
	  m_gazR(0), m_gazL(0), m_brake(0), m_steer(0), m_handBrake(false), m_engineTorque(0), m_brakePower(0),
...
void gkVehicle::updateTransmition(gkScalar rate)
{
	if (m_gearBox)
	{
		...
		if (gearRatio == 0 )
			m_currentRpm = m_ruptorRpm * m_gazL;
		else
...
void gkVehicle::updateVehicle(gkScalar rate)
{
	...
	wheelTorqueL = m_gazL * m_engineTorque * gearRatio / getNumberOfDrivenWheel();
	wheelTorqueR = m_gazR * m_engineTorque * gearRatio / getNumberOfDrivenWheel();
