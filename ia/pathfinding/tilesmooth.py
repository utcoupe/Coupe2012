...
def smooth(raw_path, collision_matrix):
	...
	if len(raw_path) > 2:
		...
		for node in raw_path[2:]:
			if walkable(check_point,node,check_point_dir, collision_matrix):
...
def walkable(node1, node2, current_dir, collision_matrix):
	if current_dir:
