...
void computeRobotState(){
	...
	//robot_state.speed = 0;
	robot_set_rad_angle(moduloPI((double)(value_right_enc-value_left_enc)/(double)ENC_CENTER_DIST_TICKS));
	robot_set_x(robot_get_x() + (double)delta_dist*cos(robot_get_angle()));
	robot_set_y(robot_get_y() + (double)delta_dist*sin(robot_get_angle()));
}
